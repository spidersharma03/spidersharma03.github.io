...
var fragmentShaderIBL = "precision highp float;\n\
  ...
  vec3 convertRGBEToRGB(vec4 rgbe) {\n\
   ...
   void main() {\n\
      ...
      float layerThickness = 0.0 * length(dn);\n\
      ...
      vec3 lSecond = -refract(-reflectionVector, normalizedWorldNormal, 1.0/n1);// Light vector for second layer\n\
      vec3 lSecondG = -refract(-reflectionVector, normalizedWorldNormal, 1.0/n2);// Light vector for second layer\n\
      vec3 lSecondB = -refract(-reflectionVector, normalizedWorldNormal, 1.0/n3);// Light vector for second layer\n\
      ...
      vec4 IblSpecularColorLayer2 = SampleSpecularContribution(specularContributionLayer2, reflectionVector,0.0);\n\
      ...
      vec4 finalColor =   IblSpecularColorLayer1 + ( diffuseContributionLayer2 + IblSpecularColorLayer2 ) * vec4(attenuation, 1.0);// + diffuseContributionLayer2;\n\
       ...
       gl_FragColor = colorRefract*1.0  + 0.2*finalColor ;// + finalColor * 0.5;\n\
      #else\n\
