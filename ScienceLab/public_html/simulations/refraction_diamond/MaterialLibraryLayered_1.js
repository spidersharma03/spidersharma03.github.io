...
var fragmentShaderIBL = "precision highp float;\n\
  ...
  vec3 convertRGBEToRGB(vec4 rgbe) {\n\
   ...
   vec4 SampleSpecularContribution(vec4 specularColor, vec3 direction, float roughness) {\n\
      ...
      #ifdef USE_HDR\n\
        vec2 uvRepeat = fract(vec2(phi_refl, theta_refl) * repeatFactor);\n\
        vec4 refractionColor = texture2D(RefractionTexture, uvRepeat);\n\
        vec4 rgbeLower = texture2D(IBLTexture, texCoordLower);\n\
        vec4 rgbeUpper = texture2D(IBLTexture, texCoordUpper);\n\
        vec3 rgbLower = refractionColor.xyz;//tonemap( specularColor.xyz*convertRGBEToRGB(rgbeLower));\n\
        vec3 rgbUpper = refractionColor.xyz;//tonemap( specularColor.xyz*convertRGBEToRGB(rgbeUpper));\n\
   ...
   void main() {\n\
      ...
      #ifdef USE_HDR\n\
       vec4 temp = IblSpecularReflectionColor*0.2 + finalColor*0.8;// + colorRefract;//colorRefract*0.5  + 0.2*finalColor ;// + finalColor * 0.5;\n\
       gl_FragColor = vec4(temp.xyz, 0.7);\n\
      #else\n\
