...
var vertexShaderIBL = "varying vec2 vUv; \n\
   ...
   projectedPos = projectionMatrix * viewMatrix * vec4(vecPos,1.0 );\n\
...
var fragmentShaderIBL = "precision highp float;\n\
  ...
  \n\
   ...
   \n\
   void main() {\n\
      vec2 uvProj = (projectedPos.xy/projectedPos.w + 1.0)*0.5;\n\
      vec4 depthFront = texture2D(DepthMapFront, uvProj);\n\
      vec4 depthBack = texture2D(DepthMapBack, uvProj);\n\
      float depthDiff = abs(depthBack.x - depthFront.x);\n\
      depthDiff = smoothstep(0.0, depthDiffMax, depthDiff);\n\
      //float derivative = fwidth(depthDiff);\n\
      //depthDiff = smoothstep(0.0, depthDiffMax, depthDiff);\n\
      //depthDiff -= derivative;\n\
      vec3 attenuation = exp(-absorptionCoeff*vec3(depthDiff));\n\
      ...
      float roughnessVal = surfaceRoughness;//(1.0 - texture2D(RoughnessMap, uvRepeat).r);\n\
      vec4 specularContribution = vec4(EnvBRDFApprox(specularColor, roughnessVal, ndotv),1.0);\n\
      vec4 IblSpecularColor = SampleSpecularContribution(specularContribution, reflectionVector,roughnessVal);\n\
      vec3 transmissionDirection = refract(viewVector, normalizedWorldNormal, 1.0/1.4);\n\
      vec4 IblTransmissionColor = SampleSpecularContribution(vec4(1.0), transmissionDirection, volumeRoughness);\n\
      vec4 diffuse1 = SampleDiffuseContribution(DiffuseColor, normalizedWorldNormal);\n\
      vec4 diffuse2 = SampleDiffuseContribution(DiffuseColor, -normalizedWorldNormal);\n\
      vec4 lightColorTransmission = IblTransmissionColor * vec4(attenuation, 1.0);\n\
      vec4 finalColor = IblSpecularColor  + diffuse1*0.4 + diffuse2 * lightColorTransmission;\n\
...
var shaderSource =
{
    uniforms: {
        ...
        surfaceRoughness: { type: 'f', value: 0.6 },
        volumeRoughness: { type: 'f', value: 0.75 },
        depthDiffMax: { type: 'f', value: 0.5 },
        absorptionCoeff: { type: 'v3', value: new THREE.Vector3(2,2,2)}
    },
